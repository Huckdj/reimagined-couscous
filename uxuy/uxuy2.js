...
class ClientAPI {
  ...
  async createUserAgent() {
    try {
      ...
      this.log(`Không thể tạo user agent, hãy thử lấy token khác: ${error.message}`, "error");
      return;
    }
  }
  ...
  async auth() {
    ...
    formData.append("start_param", "A_5730580193_inviteEarn");
  ...
  async getValidToken() {
    ...
    if (existingToken && !isExp) {
      this.log("Sử dụng token hợp lệ", "success");
      return existingToken;
    } else {
      this.log("Token không tồn tại hoặc đã hết hạn, bỏ qua...", "warning");
    }
    return null;
  }
  ...
  async handleTasks() {
    ...
    if (tasks.length === 0) {
      this.log("Không có nhiệm vụ nào để làm", "warning");
      return;
    }
    ...
    for (const task of tasks) {
      try {
        if (!task.clicked) {
          this.log(`Hoàn thành nhiệm vụ ${task.name} ...`);
        ...
        if (!verifyResult.success || !verifyResult.data?.items) {
          this.log(`Không thể xác minh nhiệm vụ ${task.name}`, "error");
          continue;
        }
        ...
        for (const verifiedTask of verifiableTasks) {
          try {
            ...
            if (resClaim.success) {
              this.log(
                `Nhận nhiệm vụ ${verifiedTask.name} thành công | ` +
                `Phần thưởng: ${verifiedTask.awardAmount}`, 
                "success"
              );
              ...
              this.log(
                `Không thể nhận nhiệm vụ ${verifiedTask.name}`, 
                "error"
              );
            }
          } catch (error) {
            this.log(
              `Lỗi nhận thưởng nhiệm vụ ${verifiedTask.id}: ${error.message}`, 
              "error"
            );
          }
        }
        ...
        this.log(`Lỗi xử lý nhiệm vụ ${task.id}: ${error.message}`, "error");
        continue;
      }
    }
  }
  ...
  async handleFarming() {
    ...
    if (farmInfo.success) {
      ...
      if (currentTime < finishTime) {
        ...
        return this.log(`Chưa đến thời gian để claim, cần chờ ${remainingMinutes} phút ${remainingSeconds} giây.`, "warning");
      }
      ...
      if (finished && !rewarded) {
        ...
        if (resClaim.success) {
          this.log(`Claim farm thành công | Phần thưởng: ${awardAmount}`, "success");
        }
        await sleep(1);
  ...
  async runAccount() {
    ...
    console.log(`[*] Tài khoản ${accountIndex + 1} | ${firstName + " " + lastName} | ${proxyStatus} | Bắt đầu sau ${timesleep} giây...`.green);
    ...
    if (!token) {
      this.log("Không tìm thấy token hoặc token đã hết hạn..bỏ qua tài khoản", "error");
      return;
    }
...
async function main() {
  ...
  console.log(`Data: ${queryIds.length} tài khoản`);
  ...
  while (true) {
    ...
    await sleep(3);
    console.log(`[*] Đã hoàn thành tất cả tài khoản, chờ ${settings.TIME_SLEEP} phút...`.magenta);
